...
// Exercice 8 : Ecrire la fonction pour afficher la liste des types de l'item.
	...
	public function listenerItem($id) {
		...
		return $this -> select(
			"i.*, c.name as categorie", // SELECT 
			"`items` i, categories c",	// FROM 
			"i.categories_idcategories = c.idcategories AND i.iditems = ". $id ." GROUP BY i.iditems"	// WHERE ... AND ... GROUP BY 
		);
	}
	...
	public function listenerReviewsItem($id){
		...
		return $this->select("note, commentaire, CONCAT(firstanme, ' ' ,lastname) as username","reviews, clients","items_iditems=$id AND clients_idclients=idclients");
	}
